...
# --- CONFIGURATION ---
st.set_page_config(
    page_title="NSE Pro Monitor v4.9 (Trend Shift Mode)",
    layout="wide",
    page_icon="📈",
)
...
def is_market_open():
  ...
  start_time, end_time = now.replace(
...
with st.sidebar:
  ...
  capital = st.number_input(
      "Capital (₹)", min_value=1000, value=20000, step=1000
  )
  ...
  filter_roc_gt = st.number_input(
      "ROC Greater Than (>) %", value=1.36, step=0.01, format="%.2f"
  )
  filter_roc_lt = st.number_input(
      "ROC Less Than (<) %", value=1.36, step=0.01, format="%.2f"
  )
  filter_trade_type = st.selectbox(
      "Trade Type Filter",
      ["All", "S.Buy Only", "S.Sell Only", "S.Buy & S.Sell", "Blank Only"],
  )
  ...
  full_list = (
      ...
      " SJVN, NLCINDIA, STAR, RAILTEL, PETRONET, SUZLON, CENTURYPLY, IGL,"
      " PNCINFRA, STARCEMENT, PPLPHARMA, JWL, JINDWORLD, HINDCOPPER, RCF, TTML,"
      " VEDL, UNIONBANK, OIL, SAREGAMA, INFY, MUTHOOTFIN, NYKAA, RALLIS,"
      " NESTLEIND, KARURVYSYA, RELIANCE, IOC, PCBL, ADANIPORTS, TANLA, GRASIM,"
      " ENGINERSIN, FEDERALBNK, TRIDENT, MOTHERSON, AMBUJACEM, FINCABLES, NMDC,"
      " TATAPOWER, BBTC, ARVIND, BANDHANBNK, ABCAPITAL, HFCL, PFC, BEL, PNB,"
      " CGPOWER, CUB"
  )
...
try:
  indices = yf.download(
      ["^NSEI", "^BSESN"], period="2d", interval="5m", progress=False
  )
  if not indices.empty and "Close" in indices.columns:
    ...
    st.markdown(
        f"### NIFTY 50: **{n_curr:,.2f}** ("
        f"{':green' if n_chg>=0 else ':red'}[{n_chg:+.2f}%]) | SENSEX:"
        f" **{s_curr:,.2f}** ({':green' if s_chg>=0 else ':red'}[{s_chg:+.2f}%])"
    )
  else:
...
def process_strategy(data, is_reversed=False):
  ...
  for symbol in SYMBOLS:
    ...
    try:
      ...
      if slope_price < 0 and slope_ad > 0:
        ...
        trend_status = (
            "🔼 Continuous" if slope_ad > 0 else "🔽 Continuous"
        )
      ...
      if len(df) >= window + 2:
        ...
        adx_series = 100 * (
            np.abs(p_di - m_di) / np.where((p_di + m_di) == 0, 1e-9, (p_di + m_di))
        )
      ...
      if use_ma20:
        sigs.append("↑MA" if ma_up else "↓MA")

      ema_up = cmp > df["Close"].ewm(span=9).mean().iloc[-1]
      if use_ema9:
        sigs.append("↑EMA" if ema_up else "↓EMA")

      if use_sma50:
        sigs.append(
            "↑SMA50"
            if len(df) > 50 and cmp > df["Close"].rolling(50).mean().iloc[-1]
            else "•SMA"
        )
      ...
      if trade:
        ...
        if (
            trade["type"] == "BUY"
            and (cmp >= trade["target"] or cmp <= trade["sl"])
        ) or (
            trade["type"] == "SELL"
            and (cmp <= trade["target"] or cmp >= trade["sl"])
        ):
        ...
        if t_type:
          p_text = (
              "LOW" if prob_score <= 1 else "MED" if prob_score == 2 else "HIGH"
          )
          entry = cmp
          target = (
              entry * (1 + target_pct)
      ...
      if filter_trade_type == "S.Sell Only" and trade_cond != "S.Sell":
        continue
      if (
          filter_trade_type == "S.Buy & S.Sell"
          and trade_cond not in ["S.Buy", "S.Sell"]
      ):
        continue
  ...
  df_out = (
      ...
      .sort_values(by=["InTrade", "ROC_Val"], ascending=False)
      .drop(columns=["InTrade", "ROC_Val"])
  )
  return df_out[[
      "Stock",
      "Trade",
...
# --- TOP GAINERS & LOSERS CALCULATOR ---
def get_gainers_and_losers(data, top_n=30):
  if data.empty or not isinstance(data.columns, pd.MultiIndex):
  ...
  for symbol in SYMBOLS:
    ...
    if t_str not in actual_present_tickers:
      continue
    try:
      df = data[t_str].dropna(subset=["Close"])
      if len(df) < 2:
        continue
      cmp = float(df["Close"].iloc[-1])
      prev_close = float(
          df["Close"].iloc[0]
      )  # Baseline close from start of window
      chg_abs = cmp - prev_close
  ...
  df_all = pd.DataFrame(changes)

  gainers = (
      df_all.sort_values(by="Change (%)", ascending=False)
      .head(top_n)
      .reset_index(drop=True)
  )
  losers = (
      df_all.sort_values(by="Change (%)", ascending=True)
      .head(top_n)
      .reset_index(drop=True)
  )

  # Dynamic styling formatters
...
def apply_dynamic_styles(df):
  ...
  for i, row in df.iterrows():
    if row["Status"] == "IN TRADE":
      ...
      styles.loc[i, :] = (
          f"background-color: {row_bg}; color: black; font-weight: 500"
      )

      is_buy = row["TradeType"] == "BUY" or (
          isinstance(row["Trade"], str) and "Buy" in row["Trade"]
      )
      if is_buy:
        ...
        cmp_bg = "#1a8a44" if row["CMP"] <= row["Entry"] else "#c53030"
      styles.loc[i, "CMP"] = (
          f"background-color: {cmp_bg}; color: white; font-weight: bold"
      )
  return styles
...
df_reg = (
    process_strategy(raw_market_data, is_reversed=False)
    if isinstance(raw_market_data, pd.DataFrame) and not raw_market_data.empty
    else pd.DataFrame()
)
df_rev = (
    process_strategy(raw_market_data, is_reversed=True)
    if isinstance(raw_market_data, pd.DataFrame) and not raw_market_data.empty
    else pd.DataFrame()
)
...
active_reg = (
    df_reg[df_reg["Status"] == "IN TRADE"]
    if not df_reg.empty
    else pd.DataFrame()
)
active_rev = (
    df_rev[df_rev["Status"] == "IN TRADE"]
    if not df_rev.empty
    else pd.DataFrame()
)
...
if not active_reg.empty or not active_rev.empty:
  ...
  combined_trades = combined_trades.drop_duplicates(
      subset=["Stock"], keep="first"
  )

  view_trade_window = combined_trades.style.apply(
      apply_dynamic_styles, axis=None
  ).format({
      "CMP": "{:.2f}",
      "Entry": "{:.2f}",
      "Target": "{:.2f}",
...
# --- 🚀 4) TOP 30 GAINERS & LOSERS ---
st.markdown("---")
st.header("📈 4) TOP 30 GAINERS & LOSERS")

df_gainers, df_losers = (
    get_gainers_and_losers(raw_market_data, top_n=30)
    if isinstance(raw_market_data, pd.DataFrame) and not raw_market_data.empty
...
with col_g:
  st.subheader("🚀 Top 30 Gainers")
  if not df_gainers.empty:
    st.dataframe(
        df_gainers.style.format({"CMP (₹)": "{:.2f}", "Change (₹)": "{:+.2f}"})
        .map(
            lambda v: (
                "color: #1a8a44; font-weight: bold"
                if str(v).startswith("+")
                else ""
            ),
    ...
    st.caption("No gainer data available.")

with col_l:
  st.subheader("🔻 Top 30 Losers")
  if not df_losers.empty:
    st.dataframe(
        df_losers.style.format({"CMP (₹)": "{:.2f}", "Change (₹)": "{:+.2f}"})
        .map(
            lambda v: (
    ...
    st.caption("No loser data available.")
